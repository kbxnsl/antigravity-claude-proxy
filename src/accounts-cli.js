...
/**
 * Load existing accounts from .env or fallback to config
 */
function loadAccounts() {
    ...
    if (existsSync('.env')) {
        try {
            ...
            // Extract accounts
            while (index <= 10) {
                const email = envVars[`ACCOUNT_${index}_EMAIL`];
                if (!email) break;

                accounts.push({
                    email,
                    refreshToken: envVars[`ACCOUNT_${index}_REFRESH_TOKEN`],
                    projectId: envVars[`ACCOUNT_${index}_PROJECT_ID`],
                    source: envVars[`ACCOUNT_${index}_SOURCE`] || 'oauth',
    ...
    try {
        ...
        console.error('Error loading accounts config:', error.message);
    }
...
function saveAccounts(accounts, settings = {}) {
    try {
        ...
        envLines.push('# Auto-generated by accounts CLI');
        envLines.push('');
        ...
        accounts.forEach((acc, index) => {
            ...
            envLines.push(`ACCOUNT_${num}_SOURCE=oauth`);
            envLines.push('');
        });

        // Add settings
        envLines.push('# Settings');
        envLines.push(`COOLDOWN_DURATION_MS=${settings.cooldownDurationMs || 60000}`);
        envLines.push(`MAX_RETRIES=${settings.maxRetries || 5}`);
        envLines.push('');

        writeFileSync('.env', envLines.join('\n'));
        console.log(`\n✓ Saved ${accounts.length} account(s) to .env file`);
    } catch (error) {
